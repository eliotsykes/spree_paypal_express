...
module Dynamo::PaypalExpress
  ...
  def confirm
    ...
    @ship_address.state = State.find_by_abbr(@address['state']) rescue ''
    if @ship_address.country.id == 35
      @ship_address.state = State.find_by_name(@address['state']) rescue ''
    end
    @ship_address.state_name = @address['state']
    @ship_address.save
    @order.checkout.update_attribute(:ship_address, @ship_address)
    @order.checkout.update_attribute(:bill_address, @ship_address)
    # or current user
    @order.checkout.update_attribute(:email, details_response.email)
    ...
    @rates = @order.shipping_methods.collect do |ship_method|
      this_rate =  ship_method.calculate_shipping(fake_shipment)
      { :id   => ship_method.id, 
     ...
     # free shipping?
          if @order.eligible_for_free_shipping?
            @rates[0][:rate] = 0
            @rates[0][:name] = @rates[0][:name] << ' (free shipping!)'
          end
  ...
  def complete
    ...
    @order.checkout.shipping_method_id = params[:method_id]
